...
void color_rectangle(std::vector<uint8_t> &image, uint32_t x, uint32_t len_x,
  ...
  for (uint32_t i = 0; i < len_y; i += units_per_pixel) {
...
void save_pgm(Data &d, std::string dir_path, std::string image_file_prefix,
  ...
  uint32_t image_width =
      d.chip_x / units_per_pixel + (d.chip_x % units_per_pixel != 0);
  uint32_t image_height =
      d.chip_y / units_per_pixel + (d.chip_y % units_per_pixel != 0);
  ...
  for (size_t i = 0; i < d.num_nets; i++) {
    ...
    for (size_t j = 0; j < n.pins.size(); j++) {
      ...
      color_rectangle(image, n_x, 1, n_y, 1, image_width, units_per_pixel,
                      PIN_COLOR);
    }
  }
  ...
  filename.append(image_file_prefix)
      .append("_")
      .append(std::to_string(step))
      .append(".pgm");
  std::ofstream file;
